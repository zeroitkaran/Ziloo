...
class nameViewController: UIViewController,UITextFieldDelegate {
    ...
    @objc func textFieldDidChange(_ textField: UITextField) {
        ...
        if textCount! > 3{
            btnSignup.backgroundColor = UIColor(named: "theme")
    ...
    func registerPhoneNo(){
        ...
        ApiHandler.sharedInstance.registerPhone(phone: phoneNo, dob: dob, username: username, device: deviceType) { (isSuccess, response) in
            if isSuccess{
...
//                        NotificationCenter.default.post(name: Notification.Name("dismissVC"), object: nil)
    ...
    func registerEmail(){
        ...
        print("gender:" , gender)         
        AppUtility?.startLoader(view: self.view)
        ApiHandler.sharedInstance.registerEmail(email: email, password: pass, dob: dob, username: username, device: deviceType, gender: gender) { (isSuccess, response) in
