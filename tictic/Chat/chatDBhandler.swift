...
class ChatDBhandler
{
    ...
    func sendMessage(senderID: String, receiverID: String,senderName: String, message: String, seen: Bool, time: Any,timestamp: Any, type: String, completionHandler: @escaping (_ result: Bool) -> Void)
    {
        let messages  = Database.database().reference().child("chat")
        let chatOf = messages.child(senderID+"-"+receiverID).childByAutoId()
        let value = ["chat_id": "\(chatOf.key!)", "text": "\(message)","pic_url":"","sender_id":senderID,"receiver_id":receiverID,"sender_name":senderName ,"seen": seen, "time": time, "timestamp": timestamp, "type": "\(type)"] as [String : Any]
...
//                    AppUtility.shared.hideLoader()
                ...
                else
                {
                    ...
                    */
                    ...
                    self.sendMessage(senderID: senderID, receiverID: receiverID, senderName: senderName!, message: "\(url!)", seen: seen, time: time, timestamp: time, type: type) { (isSuccess) in
                        completionHandler(true, "\(url!)")
//                         AppUtility.shared.hideLoader()
                    }
                }
...
//                AppUtility.shared.hideLoader()
    ...
    func sendPushNotification(to token: String, title: String, body: String)
    {
        ...
        request.setValue("key=AAAADCdy7j4:APA91bGnj9_4JU3I8KPFDIh8Sf_qqH8BSvgG-Kyg4VW3nmRYYVooEZwCWLhEYLn76QowJIb7RS9pdMV1oIP5tYSzYfx-jJjy90GrKnuGLQSL3QuaerhPO6k29o85Lpnfmx3g-eB1HwYL", forHTTPHeaderField: "Authorization")
